/**
 * Script to update ALL components to use global settings
 * Run with: node update-all-components.mjs
 */
...
const allTsxFiles = execSync('find src -type f -name "*.tsx"')
  .toString()
  .split('\n')
  .filter(Boolean);

console.log(`Found ${allTsxFiles.length} TSX files to process.`);
...
// List of files to skip (files that shouldn't be modified)
const skipFiles = [
...
function processFile(filePath) {
  ...
  // Additional Tailwind class replacements for key components
  
  // Replace Tailwind text colors
  const textColorRegex = /(className=["'].*?)(text-(?:red|blue|green|purple|indigo|pink|yellow|gray|slate|neutral)-\d+)(.*?["'])/g;
  if (content.match(textColorRegex)) {
    content = content.replace(textColorRegex, (match, prefix, colorClass, suffix) => {
      // Check if it's a header or title
      if (match.includes('heading') || match.includes('title') || match.includes('h1') || 
          match.includes('h2') || match.includes('h3') || match.includes('h4')) {
        return `${prefix}text-primary${suffix}`;
      } else {
  ...
  // Replace Tailwind background colors
  const bgColorRegex = /(className=["'].*?)(bg-(?:red|blue|green|purple|indigo|pink|yellow|gray|slate|neutral)-\d+)(.*?["'])/g;
  if (content.match(bgColorRegex)) {
    content = content.replace(bgColorRegex, (match, prefix, colorClass, suffix) => {
      // Check context to determine best replacement
      if (match.includes('button')) {
        return `${prefix}bg-primary${suffix}`;
      } else if (match.includes('card') || match.includes('panel') || match.includes('section')) {
        return `${prefix}bg-background${suffix}`;
      } else if (match.includes('accent') || match.includes('highlight')) {
        return `${prefix}bg-accent${suffix}`;
      } else {
        return `${prefix}bg-background${suffix}`;
      }
    });
...
allTsxFiles.forEach(targetPath => {
  try {
    if (skipFiles.includes(targetPath)) {
      console.log(`🔵 Skipping ${targetPath} (in skip list)`);
      skippedCount++;
      return;
    }
...
========================
    UPDATE COMPLETE
========================
✅ Successfully processed: ${updatedCount} files
🔵 Skipped: ${skippedCount} files
❌ Errors: ${errorCount} files

Please rebuild your app with: npm run build
`);

// Add a quick summary of files updated to a log file
const summary = `
Global Theming Update Summary
-----------------------------
...
fs.writeFileSync('theming-update.log', summary, 'utf8');
console.log('Created theming-update.log with summary information.'); 